...
/*   Updated: 2019/04/03 19:53:49 by aducimet    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
void ft_side_dist(t_wolf *wolf)
{
	...
	{
		wolf->step.x = 1;
		wolf->sidedist.x = (wolf->mapx + 1.0 - wolf->raypos.x) * wolf->deltadist.x;
	}
	if (wolf->raydir.y < 0)
	{
		wolf->step.y = -1;
	...
	{
		wolf->step.y = 1;
		wolf->sidedist.y = (wolf->mapy +1.0 - wolf->raypos.y) * wolf->deltadist.y;
	}
}

void 	ft_find_ray_dist(t_wolf *wolf)
{
	wolf->deltadist.x = sqrt(1 + (wolf->raydir.y * wolf->raydir.y) / (wolf->raydir.x * wolf->raydir.x)); // a modifier (creer fonction)
	wolf->deltadist.y = sqrt(1 + (wolf->raydir.x * wolf->raydir.x) / (wolf->raydir.y * wolf->raydir.y));
	ft_side_dist(wolf);
}
...
void 	define_ray(t_wolf *wolf, int x)
{
	...
	ft_ray_check(wolf);
	if (wolf->type == 0)
		wolf->walldist = ((wolf->mapx - wolf->raypos.x + (1 - wolf->step.x) / 2) / wolf->raydir.x) + 0.001;
	else
		wolf->walldist = (wolf->mapy - wolf->raypos.y + (1 - wolf->step.y) / 2) / wolf->raydir.y + 0.001;
...
void    ft_draw(t_wolf *wolf)
{
	...
	mlx_put_image_to_window(wolf->mlx_ptr, wolf->win_ptr, wolf->img.ptr_img,0 ,0);
	ft_hud(wolf);
}
